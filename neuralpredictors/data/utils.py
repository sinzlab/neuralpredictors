...
def _savenpy(path, val, overwrite):
    if (
        not np.isscalar(val) and len(val.shape) > 0 and val[0].dtype.char == "S"
    ):  # convert bytes to univcode
        val = val.astype(str)
...
def convert_static_h5_dataset_to_folder(
