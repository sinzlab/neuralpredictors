...
class DeterministicGaussian2d(nn.Module):
    ...
    From the mean and and variance, a filter unique for each outdim is build, that is of the 2d density of the normal distribution. The filter is
    applied only on the spatial dimensions of the input.
    Args:
        in_shape (list): shape of the input feature map [channels, width, height]
        outdims (int): number of output units
        bias (bool): adds a bias term
    ...
    def __init__(self, in_shape, outdims, bias, constrain_positive=False, constrain_mode='elu', **kwargs):
        ...
        self.log_var = Parameter(
            data=torch.zeros(outdims,), requires_grad=True)
