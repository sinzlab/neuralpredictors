...
class AffineFullGaussian2d(nn.Module):
    """
    A readout using a spatial transformer layer whose positions are sampled from one Gaussian per neuron. Mean
    and covariance of that Gaussian are learned.
    ...
    def __init__(self, in_shape, outdims, bias, init_mu_range=0.1, init_sigma=1, batch_sample=True,
        ...
        # position grid shape
        self.grid_shape = (1, outdims, 1, 2)
        self.grid_mean_predictor = grid_mean_predictor
        # the grid can be predicted from another grid
        self._predicted_grid = False
        self._shared_grid = False
        self._original_grid = not self._predicted_grid
        ...
        if grid_mean_predictor is None and shared_grid is None:
            ...
            self.angle = nn.Parameter(torch.tensor([0.]))
            self._mu_transform = nn.Parameter(torch.tensor([[torch.cos(self.angle), -torch.sin(self.angle)],
    ...
    # def cortex_orthogonality_regularizer(self):
    #     if self.grid_mean_predictor is not None:
    #         return torch.mm(self.mu_transform[0].weight[:, 0].view(1, -1),
    #                         self.mu_transform[0].weight[:, 1].view(1, -1).T).abs()
    #     else:
    #         return 0.
    #
    # def cortex_length_ratio_regularizer(self):
    #     if self.grid_mean_predictor is not None:
    #         return torch.abs(torch.norm(self.mu_transform[0].weight[:, 0], p=2) -
    #                          torch.norm(self.mu_transform[0].weight[:, 1], p=2))
    #     else:
    #         return 0.
    ...
    def mu_transform(self, source_grid):
        return torch.mm(self._mu_transform, source_grid.T).T * self.scale + self.offset


    def init_grid_predictor(self, source_grid, hidden_features=20, hidden_layers=0, final_tanh=False):
        self._original_grid = False

        # Todo: This is hacky and affine=False probably doesnt work
