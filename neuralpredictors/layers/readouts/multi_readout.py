...
class MultiReadoutBase(torch.nn.ModuleDict):
    ...
    def __init__(self, in_shape_dict, n_neurons_dict, base_readout=None, mean_activity_dict=None, clone_readout=False, gamma_readout=1., **kwargs):
        ...
        for i, data_key in enumerate(n_neurons_dict):
            ...
            if i == 0 or clone_readout is False:
                self.add_module(
                    data_key,
                    self._base_readout(in_shape=in_shape_dict[data_key], outdims=n_neurons_dict[data_key], mean_activity=mean_activity, reg_weight=gamma_readout, **readout_kwargs),
                )
